...
import 'src/scss/orderConfirm.scss';

const ctrlAPI ={
	orderData:{ url:"/api/trade-init.html", type:"get" },
	coupon:{ url:"/api/trade-avail-coupon.html", type:"get" },
	applyCoupon:{url:"/api/trade-apply-coupon.html",type:"post"},
	address:{ url:"/api/trade-order-total.html", type:"post"},
	submit:{ url:"/api/trade-order-create.html", type:"post"},
...
export default class OrderConfirm extends Component {
  constructor(props) {
	...
	componentDidMount() {
		$(this.refs.orderPage).css({height:$(window).height(),overflow:"auto"} );
	...
	render() {
		...
		return (
			<div data-page="order-confirm" ref="orderPage">
				...
				<ModalAComp active={ data && data.order && data.order.cart && !data.order.cart.length } msg={"对不起，系统繁忙，请稍后再试！"} btns={[{text:"返回",cb:()=>{ browserHistory.replace('/shopCart') } }] }/>
				...
				<ModalAComp active={address} msg={"您还没有收货地址，快去添加吧！"}
				            btns={[
					            ...
					            { text:"确定", cb: ()=>{  window.location="trc://getAddressInfo" } }
				            ]}
				/>
...
class OrderHeader extends Component{
	...
	render(){
		...
		<div className="order-header">
			<a href="trc://getAddressInfo" className="user-address g-row-flex g-col-mid g-col-ctr">
			...
			{idNeed ?
				<a href={`trc://getIdCardInfo?status=${data.order && data.order.is_over_seas}`} className="user-idtf g-row-flex g-col-ctr g-col-mid">
					<div className="left-icon">
...
export class CouponList extends Component{
	...
	//优惠券适用范围
	couponApplyRange( coupon,shop,category,brand) {
		if( !coupon ){
			return "指定商品适用";
		}
		if( shop ==="self" ){
			if( category!=="all" ){
				if( brand !== "all"){return "自营商品指定分类、品牌适用";}
				else{ return "自营商品指定分类适用"; }
			}else{
				if( brand !== "all"){return "自营商品指定品牌适用";}
				else{return "指定自营商品适用";}
			}
		}else{
			if( category!=="all" ){
				if( brand !== "all"){return "指定分类、品牌适用";}
				else{ return "指定分类适用"; }
			}else{
				...
				return (
					<div className={`one-coupon g-row-flex ${colour}`} key={`coupon${i}`} onClick={ ()=> { self.props.onCoupon( item.coupon_code,+item.limit_money,+item.deduct_money );} } >
						...
						<div className="right g-col-1">
							...
							<p className="two">{self.couponApplyRange( item.coupon_type, item.used_shop_type, item.used_category, item.used_brand ) }</p>
