...
const ctrlAPI ={
	...
	orderNum:{ url:"/api/wap/trade-morderCount.html", type:"get" },
	couponNum:{ url:"/api/wapapi/coupon.api?count_no_use=1",type:"get"}
};
	...
	render(){
		...
		return(
			<section className="person-center-header" >
				<a href="trc://setting" className="setting-btn" > </a>
				<a className="user-header c-tc" href={isLogin?"trc://userinfo":"taihe://to_login"}>
					<div className="header-img"><img src={userInfo.avatar?userInfo.avatar:"/src/img/personCenter/head-img-default.png"} /></div>
...
export  class UserOrder extends Component{
	...
	render(){
		...
		return(
			<div>
				<LinkOrA className="user-order c-clrfix"  link={isLogin?"/tradeList/0":"taihe://to_login"} type={isLogin?"link":"a" }>
					<div className="c-fl">
	...
	const { num, item,login } = this.props;
		return(
			<div className="order-icon c-tc">
				<LinkOrA link={login?item.url:"taihe://to_login"} type={login?"link":"a" } >
		...
		let gridViewData = this.props.data.map(( item,i)=>{
			return(
				<LinkOrA key={i} className={item.imgClass}
				         link={isLogin?item.url:"taihe://to_login"}
