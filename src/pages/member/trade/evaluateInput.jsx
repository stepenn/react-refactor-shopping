    ...
    return(
      ...
      <div data-page="evaluate-list">
			<section ref="evaluate-suc">
			  {this.state.evaluateAlready==='success'?<EvaluateSuccess />:<EvaluateIput dataList={dataList} fn={this.submiteEva} />}	
			  <div id="tips" className="c-fs14 c-lh30 c-tc c-bg000 c-cfff c-pf" style={{top:'150px',left:'50%',marginLeft:'-80px',width:'160px',height:'30px',opacity:'0.8',borderRadius:'15px',zIndex:'200',display:'none'}}>最多只能输入100字</div>
			</section>
		  </div>
		  :<LoadingRound />
    )
...
class EvaluateSuccess extends Component {
	...
	componentDidMount(){
		let self=this;
		$.ajax({
      url: "/api/wapapi/recommendItem.api",
      type: "get",
      success: function(data) {
      	if(data.code === 200 && data.status === true){
	    		self.setState({
    ...
    return(
      <div className="each-goods">
				<div className="goods-position">
				  <Link to={'/item?item_id='+data.item_id} className="goods-img"><img src={data.image} /></Link>
  ...
  render() {
    return(
      <div className="find-more">
				<a href="trc://main?page=home" className="c-c35 to-more">查看更多></a>
		  </div>
    )
  }
}
...
class ShopsEva extends Component {
	render(){
		let {dataList} = this.props;
		let goods = dataList.good_orders.map(function (item,i) {
	...
	render(){
		...
		return(
			<div style={{width:'100%'}} id={dataList.id} className="each-goods-eva">
				<Description imageUrl={dataList.image} goodsID={dataList.item_id} />
				<EvaluateTextarea />
  ...
  render() {
    ...
    return(
      <div className="description">
				<div className="goods-img c-fl"><Link to={'/item?item_id='+goodsID}><img src={imageUrl} /></Link></div>
		  	<div className="c-fl" style={{width:'76%'}}><Stars data="描述相符" starTxt={starTxt} starClass="star-a" spanClass="description-txt" /></div>
		  </div>
    )
  }
}
...
class Stars extends Component {
  componentDidMount(){
		...
		$(tar).click(function (e) {
			...
			//先恢复初始状态
			$(this).parent().find('li').css({'background-position':'0 0'});
			//使用循环,给当前索引及前面的星星换背景
			for(var i=0;i<=cindex;i++) {
				$(this).parent().find('li').eq(i).css({'background-position':'0 -32px'});
			}
      //改变文字内容
      let msg = "";
			switch(cindex) {
...
class EvaluateImgs extends Component {
  ...
  render() {
  	...
  	let imgs = arr.map(function (item,i) {
  		return <EachImg data={data2} id={'img'+self.props.id+i} pid={self.props.id} urls={item} fn2={self.removeImg} key={i} />
  	})
    return(
      <div className="evaluate-imgs c-clrfix c-mb10">
				{imgs}
				{this.state.dataLength>4?'':<EachImg data={data1} pid={self.props.id} dataLength={this.state.dataLength} fn={this.addImg} />}
		  </div>
    )
  }
}
...
class EachImg extends Component {
  ...
  render() {
    ...
    return(
      <div id={id?'img'+id:''} className="each-img c-fl c-mr10" style={this.props.data?{border:"none"}:{}}>
				{(this.props.data)?<HasImg urls={this.props.urls} />:<HasNotImg dataLength={dataLength} fn={this.props.fn} pid={pid} />}
				{this.props.data?<div className="del-img"><div className="del"></div></div>:""}
		  </div>
    )
  }
}
...
class HasImg extends Component {
  render() {
    return <div style={{width:'80px',height:'80px',overflow:'hidden'}}><img style={{width:"80px"}} src={this.props.urls} /></div>
  }
}
	...
	render(){
		...
		return(
			<div className="shop-score" id={dataList.id}>
				<img className="shop-icon" src="/src/img/icon/store-icon.png" /><div className="score c-fs14 c-c35 c-mb14">{dataList.shop_name}</div>
				<div className="c-clrfix" style={{width:'90%'}}><Stars className="c-mb14" data={"描述相符"} starTxt={starTxt} starClass="star-b" spanClass={"shop-stara"} /></div>
				<div className="c-clrfix" style={{width:'90%'}}><Stars className="c-mb14" data={"服务态度"} starTxt={starTxt} starClass="star-c" spanClass={"shop-starb"} /></div>
				<div className="c-clrfix" style={{width:'90%'}}><Stars className="c-mb14" data={"发货速度"} starTxt={starTxt} starClass="star-d" spanClass={"shop-starc"} /></div>
				<div className="c-clrfix" style={{width:'90%'}}><Stars className="c-mb14" data={"物流速度"} starTxt={starTxt} starClass="star-e" spanClass={"shop-stard"} /></div>
			</div>
		)
	}
}
...
class SubmitEvaluation extends Component {
  		...
  		if(!self.state.subClick){
	  		...
	  		for(let i=0,len=goods.length; i<len; i++){
	  			...
	  			rate.order_id = item.attr('id');
					...
					rate.images = imgArr;
					rate.experience = item.find('.description-txt').html();
	  			switch (rate.experience){
	  				case '好评': rate.experience = 'Good';
	  					break;
	  				case '中评': rate.experience = 'Neutral';
	  					break;
	  				case '差评': rate.experience = 'Bad';
	  					break;
	  				default:rate.experience = 'Good';
	  					break;
	  			};
	  			rates.push(rate);
	  		};
  ...
  submitEach = (msg)=>{
  	...
  	$.ajax({
  		type:"post",
  		url:"/api/wapapi/doCreateRate.api",
  		data:msg,
  		success:function (data) {
  			if(data.status === true){
			  	self.props.fn();
  ...
  render() {
    ...
    return (
    	<div className="submit-evaluation">
    		<p className={isAnonymous?"anonymous c-fl c-c35 c-fs12":"anonymous c-fl c-c35 c-fs12 anonymous-cbg"} onClick={this.changeAnonymous}>匿名</p>
