...
import { DropDownLoad } from 'component/highComp.jsx';
...
const listAPI ={
	init:{ url:"/api/wap/trade-mlist.html",type:"get"},
...
export class OneListInfoCtrl extends Component{
	...
	render(){
		...
		return (
			<div className="swiper-slide" ref="oneList" style={{overflow:'scroll',height:document.body.clientHeight -40 }}>
				{update?
	...
	render(){
		...
		return(
			<li className="one-order">
				...
				{this.props.showCtrl ? <OrderCtrl type={data.type} onDelete={this.props.onDelete }  status={data.status} tid={data.id} virtual={data.is_virtual} /> :""}
			</li>
		)
	}
}
		...
		switch (status){
			...
			case "待收货":
				btnGroup = [ !virtual && <Link className="ctrl-red pay-order" key="1" to={`/logistics?tid=${tid}`}>查看物流</Link>,
					<a className="ctrl-red pay-order" key="2" onTouchTap={ this.confirmRece } href="javascript:;">确认收货</a>];
				break;
			case "待评价":
