		    	...
		    	if(self.context.isLogin){
					if($(this).attr('class')==='recharge-btn' && self.state.canGet){
						...
						if(self.state.status === 'phone'){
							...
							let num = $('#oil-num').val();
							let name = 'oil_sin_'+$('.oil-page .choose .amount').html();
...
class OilPage extends Component{
	...
	findChoose=()=>{
		...
		if(o_bills.length>=3){
			$('.oil-page .bill-box').eq(2).addClass('choose');
		} else if (o_bills.length<3) {
			$('.oil-page .bill-box').eq(o_bills.length-1).addClass('choose');
		} else if (o_bills.length===0) {
			//..
		}
	};
	render(){
		...
		return(
			<div className="oil-page" ref="oil">
				<div className="kinds">
					<div id="cn" className="oCard c-fl c-pr choose">
					...
					<div id="sin" className="oCard c-fr c-pr">
						<img src="src/img/logistics/logo2.png" />
						<p className="c-fs14">中国石油</p>
						<p className="c-c999">暂未开通</p>
						<img className="gou" src="src/img/logistics/gou.png" />
					</div>
				</div>
				<CardMsg />
				<OilBill data={data} />
				<p className="c-tc c-fs10" >支付即表示同意<a className="c-cblue" style={{textDecoration: 'underline'}} href="https://www.trc.com/wap/content-info.html?article_id=103">《中国石化加油卡客户网上充值协议》</a></p>
			</div>
		)
	}
}
		...
		return(
			<div className="card-msg">
				<div className="input-number sin">
					<input id="oil-num" type="number" placeholder="请输入19位油卡" />
...
class OilBill extends Component{
	render(){
		let {data} = this.props;
		let keyArr = [],valueArr=[],html='';
		let oilData = data.sin?data.sin.oil:'';
		if(oilData){
