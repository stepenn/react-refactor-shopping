...
export default class EvaluateList extends Component {
  ...
  addList=(id)=>{
      ...
      success: function(data) {
      	if(data.code === 200 && data.status === true){
      		let arr = self.state.listData.concat(data.data.rates);
      		self.setState({
      			page:self.state.page+1,
	          listData: arr
	        });
	        if(data.data.rates.length<20){
	        	$('.add-more').html('没有更多了')
	        } else {
  ...
  componentDidMount() {
    let self = this;
    $(self.refs.evaluate).css({ minHeight: $(window).height() });
  };
  render() {
  	let {evaluateData,listData,page} = this.state;
    ...
    return(
      ...
      <div data-page="evaluate-list">
				<section  id="evaluate-list" ref="evaluate">
				  ...
				  <Evaluate dataList={evaluateData} page={page} listData={listData} itemID={itemID} fn={this.getList} addFn={this.addList} />
				</section>
			</div>
			:<LoadingRound />
    )
  ...
  render() {
    ...
    return(
      <div className="item-msg">
				...
				<span className="item-price c-cdred c-fs14"><span className="c-fs10">￥</span>{dataList.price}</span>
			</div>
    )
  }
}
...
class Evaluate extends Component {
  render() {
  	let{dataList,itemID,listData,page}=this.props;
  	let {fn,addFn} = this.props;
    return(
      <div className="item-evaluate">
				{dataList.total?<HasEvaluate dataList={dataList} page={page} listData={listData} itemID={itemID} fn={fn} addFn={addFn} />:<NoneEvaluate />}
			</div>
    )
  }
}
  ...
  render() {
    ...
    return(
      <div style={{width:'100%',overflow:'hidden'}}>
				...
				<AddMore dataLength={dataList.rates.length} itemID={itemID} addFn={addFn} dataList={dataList} page={page} />
			</div>
    )
  }
}
	...
	render() {
		...
		return(
			<div style={{width:'100%',height:'30px',position:'relative'}}>
				...
				<div className="c-tc c-c999 c-fs14 add-more" style={{width:'100%',lineHeight:'30px'}}>
					{dataLength<20?"没有更多了":"点击加载更多"}
				</div>
			</div>
		)
	}
      ...
      <div className="list-header" style={{width:'100%'}}>
				<div className="stars c-fs14">
					...
					<div className="c-fl c-mr20 stars-bg">
						<img className="stars-img" src="/src/img/evaluate/stars.png" style={{backgroundSize:dataList.praise_rate+' 100%'}} />
					</div>
  ...
  componentDidMount() {
    ...
    $('.list-label').click(function (e) {
    	if($(this).attr('class') === "list-label active-label"){
    		...
    		let id = $(this).attr('id');
	    	self.state.fn(id);
	    	$(this).css({ 'color': '#fff', 'background-color': '#f88' }).addClass('active-label').siblings().css({ 'color': '#353535', 'background-color': '#fff2f5' }).removeClass('active-label');
    	}
    	e.preventDefault();
    })
  };
...
class OpenLabels extends Component {
  ...
  render() {
    return <div onClick={this.changeImg} className={this.state.isClose?"open-labels to-open":"open-labels to-close"}></div>
  };
}
...
class EachLabel extends Component {
  render() {
    ...
    return(
      <button className={babelChose?"list-label active-label":"list-label"} id={data.rate_tag_id}>{data.text}{data.count?'('+data.count+')':''}</button>
    )
  }
}
...
class List extends Component {
  render() {
    var list = this.props.listData.map(function(item, i) {
      return <OneInList data={item} key={i} />
    })
    return(
      ...
      <div className="one-evaluate" id={data.rate_id}>
      	{data.amazing?<Amazing />:''}
				...
				<div className="thumbs-up">
					<div className={this.state.isThumbsUp?"zan zan-already c-fr c-tc":"zan zan-yet c-fr c-tc"} onClick={this.zanClick}>{zanCount?zanCount:'点赞'}</div>
				</div>
			</div>
    )
  }
...
class UserMsg extends Component {
	getDate=(tm)=>{ 
		...
		let ttyear = tt.getFullYear(),
				ttmonth = (tt.getMonth()+1)>9?(tt.getMonth()+1):('0'+(tt.getMonth()+1)),
				ttday = tt.getDate()>9?tt.getDate():('0'+tt.getDate()),
				tthour = tt.getHours()>9?tt.getHours():('0'+tt.getHours()),
				ttmin = tt.getMinutes()>9?tt.getMinutes():('0'+tt.getMinutes());
		let couponTime = ttyear+"-"+ttmonth+"-"+ttday+' '+tthour+':'+ttmin;
		return couponTime; 
	};
  render() {
  	let {data} = this.props;
    ...
    return(
      <div>
				...
				<div className="date-type">
					<span>{data.create_time?this.getDate(data.create_time):''}</span>
...
class ContentTxt extends Component {
  render() {
    ...
    return(
      <div className="content-txt" style={{overflowWrap:'break-word',wordBreak:'break-all',wordWrap:'break-word'}}>
    ...
    let timer = null;//定时器
  	let largeSwiper = new Swiper('.swiper2'+rateID,{
		...
		$('.swiper1'+rateID+' img').click(function () {
		    $(this).parent().parent().parent().parent().next().css({display:'block'}).find('img').css({top:0,left:0,right:0,bottom:0,margin:'auto'});
		});
  };
  render() {
    ...
    return(
      <div data-plugin="swiper" className="content-imgs c-mb14" ref="imgs">
      	<div className={"swiper-container small-imgs swiper1"+rateID}>
				...
				<div style={{display:(0?'block':'none')}} className={"swiper-container large-img swiper2"+rateID}>
					<div className="swiper-wrapper">
						{larges}
					</div>
